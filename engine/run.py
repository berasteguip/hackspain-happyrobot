...
Dos formas de invocarlo, las dos válidas:
    ...
    # desde engine/ (forma de script, la que usa `make engine`)
...
# módulos fallan. Metemos la raíz del repo en sys.path y usamos imports absolutos.
...
def wait_for_api(client: CrisisApiClient, console: Console, attempts: int = HEALTH_WAIT_ATTEMPTS) -> bool:
    ...
    Requisito explícito del proyecto: el motor arranca aunque `api/` no esté levantada.
    """
    if client.dry_run:
        return False
...
def main(argv: list[str] | None = None) -> int:
    ...
    if not scenario_path.exists():
        # Comodidad: `make engine` hace `cd engine`, pero alguien puede lanzarlo desde la raíz.
        ...
        if alt.exists():
            ...
            print(
                ...
                or "       (ninguno en engine/scenarios/)",
                file=sys.stderr,
            )
            return 2

...
1. **El Python Sandbox no tiene red saliente.** Módulos permitidos: `math, datetime, pytz, re, dateutil, random, collections, json, _strptime, time, base64`. Así que el Sandbox sirve para **formatear, ordenar y validar**, nunca para hablar con `backend/api/`. Todo HTTP va por nodos **Webhook GET/POST**.
...
los workflows terminan en un `Webhook POST` a `backend/api/`**: así nuestro sistema se enrama sin tener que
preguntar.
...
**Trigger:** `Webhook / API`. Lo dispara `backend/engine/` cuando se declara la zona de evacuación.
...
**Por qué el orden lo pone el Sandbox y no el Loop:** el Sandbox recibe la lista ya puntuada por `backend/api/` y
...
> Si da problemas, WF-0 se borra y **el fan-out lo hace `backend/engine/`**: un `POST` al webhook de WF-1 por casa.
...
| `send_gps_link` | Send SMS | — (SMS de la plataforma) | manda el enlace de `frontend/gps/` en cuanto hay consentimiento |
...
**Trigger:** `Webhook / API`, disparado por `backend/api/` cuando el `decision_log` escribe un
...
- **El `Paths` de `say_this` vacío va antes del Agent.** Si `backend/api/` no ha podido generar la frase, la llamada
...
Si la integración no está conectada a tiempo, `backend/api/` usa distancia en línea recta y se dice en la demo.
...
> implementa fuera: `backend/api/` guarda el `say_this` que emitió y lo compara con `transcript_url` en un test del
...
| `Redis` | **no se usa** | ✅ consta; no hace falta, el estado vive en `backend/api/` |
| Northstars | §7 | ✅ §2 y §8 |
...
1. **Túnel público** (`PUBLIC_BASE_URL`, ngrok o cloudflared) para que los nodos Webhook alcancen nuestra `backend/api/` en el portátil. Es el punto de fallo número uno de la integración.
...
| `Loop` no hace lo que creemos | Plan B: fan-out desde `backend/engine/`. 10 líneas. |

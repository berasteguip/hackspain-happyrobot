...
> ni un campo más, ni uno menos.** Fuente: `docs/06-producto/03-contrato-de-datos.md` §3 + `backend/api/models.py`
...
Enumeración de las dos partes, una al lado de la otra. Izquierda: lo que el contrato y `backend/api/models.py`
...
añaden **primero** a `docs/06-producto/03-contrato-de-datos.md` y a `backend/api/models.py`, y luego aquí.
...
- Si has puesto algo en declared_location, estos dos siguen siendo null salvo que hayas oído los
  números.
La conversión de texto a coordenadas la hace backend/api/, que tiene el mapa. Tú no lo tienes.
```
...
- null y false NO son lo mismo y backend/api/ los trata distinto: false es una negativa registrada (y hay que
...
- Las evasivas van a false, no a true y no a null: para el sistema, una salida sin confirmar NO
  puede contar como confirmada. backend/api/ mapea false a estado `refusing` y la vuelve a llamar con otro
...
| **El esquema y el contrato se separan** | El payload se rechaza o se pierden campos en silencio. | Este cotejo (§1) se repite si `backend/api/models.py` cambia. Hoy: 12 = 12. |

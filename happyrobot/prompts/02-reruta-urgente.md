...
> carretera**. Trigger: `Webhook / API` disparado por el motor de escenario a través de `backend/api/`.
...
Esa frase la produce `backend/api/`, que es lo único que conoce el polígono del fuego, el corte de carretera, la
...
El mismo nodo Agent sirve para las tres; lo que cambia es el `say_this` que devuelve `backend/api/` y una línea de
...
⚠️ **Nota para quien implemente `say_this` en `backend/api/`** (fuera de mi alcance, pero condiciona este guion):
...
| **Llega la llamada cuando ya ha pasado el cruce** | La instrucción es imposible de cumplir y confunde. | `backend/api/` recalcula con la posición GPS de los últimos 5 s antes de generar `say_this`; el agente no lo puede arreglar. Si la persona dice "ya he pasado ese cruce" → se trata como información de campo: se registra en `agent_notes` y el sistema recalcula (segunda reruta). |

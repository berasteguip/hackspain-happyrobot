...
import type { HrCallsPulse, HrEscalationRun, HrView } from './hrModel'
...
export function CommandCenter({ token }: { token: string }) {
  ...
  const hrView: HrView = escalation ? 'escalation' : selected ? 'person' : panel ?? (hrCalls && (hrCalls.open > 0 || campaignRunning) ? 'campaign' : 'overview')
  ...
  return (
    <div className={`map-app${panel || selected ? ' has-panel' : ''}${hrCard !== 'hidden' ? ' has-hr' : ''}`}>
      ...
      {hrCard !== 'hidden' && <HappyRobotCard view={hrView} connected={apiRoster} live={liveMode && !DEMO_ONLY} calls={hrCalls} escalation={escalation ? { run: escalation, units: escalationUnits } : null} collapsed={hrCard === 'collapsed'} onToggleCollapse={() => setHrCard(value => value === 'collapsed' ? 'open' : 'collapsed')} onClose={() => setHrCard('hidden')} />}

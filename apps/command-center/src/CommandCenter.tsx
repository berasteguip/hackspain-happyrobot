...
export function CommandCenter({ token }: { token: string }) {
  ...
  return (
    <div className="map-app">
        ...
        <nav className="incident-list" aria-label="Incendios activos">
          {SCENARIOS.map(item => (
            <button type="button" key={item.id} aria-pressed={item.id === scenario.id} onClick={() => selectScenario(item.id)}>
              ...
              <span><strong>{item.incident.name}</strong><small>{item.incident.area}</small></span>
            </button>
          ))}
        </nav>
      </header>
      ...
      <section className="simulation-dock campaign-dock" aria-label="Llamadas">
        {drawingArea ? (
          <>
            <p className="campaign-heading"><strong>Arrastra el círculo</strong></p>
            <div className="campaign-actions"><button type="button" className="cop-secondary" onClick={() => { setDrawingArea(false); updateArea(null) }}>Cancelar</button></div>
          </>
        ) : campaignRunning ? (
          <>
            <p className="campaign-heading"><strong>{counts.answered}/{campaignIds.length} respondidas</strong></p>
            <div className="campaign-stats" role="status">
              <span>{counts.moving} en movimiento</span>
              <span>{counts.silent} sin respuesta</span>
              {counts.waiting > 0 && <span>{counts.waiting} sin ruta</span>}
              {planningCount > 0 && <span>Rutas {planningCount}</span>}
            </div>
            <div className="campaign-actions">
              <button type="button" className="campaign-pause" onClick={() => setProtocolOn(active => !active)} aria-label={protocolOn ? 'Pausar' : 'Reanudar'}><Icon name={protocolOn ? 'pause' : 'play'} />{protocolOn ? 'Pausar' : 'Reanudar'}</button>
              {counts.waiting > 0 && <button type="button" className="cop-secondary" onClick={retryRoutes}>Reintentar rutas</button>}
            </div>
            {areaIds.length > 0 && <DispatchActions kinds={['ambulance', 'police', 'fire']} disabled={units.length >= MAX_UNITS} onDispatch={kind => {
          ...
          <>
            <p className="campaign-heading"><strong>{areaIds.length} en zona · {Math.round(callArea.radiusM)} m</strong></p>
            ...
            <div className="campaign-actions">
              <button type="button" className="cop-primary" onClick={launchAreaCampaign} disabled={!callableCount}>{callableCount ? `Llamar ${callableCount}` : 'Sin pendientes'}</button>
              <button type="button" className="cop-secondary" onClick={() => updateArea(null)}>Borrar</button>
              {counts.waiting > 0 && <button type="button" className="cop-secondary" onClick={retryRoutes}>Reintentar rutas</button>}
            </div>
            {areaIds.length > 0 && <DispatchActions kinds={['ambulance', 'police', 'fire']} disabled={units.length >= MAX_UNITS} onDispatch={kind => {
              const target = targetFromCitizens(areaIds, callArea, `${areaIds.length} en zona`)
              if (target) dispatchUnit(kind, target)
            }} />}
          </>
        ) : (
          <>
            <p className="campaign-heading"><strong>Llamadas</strong></p>
            <div className="campaign-actions">
              <button type="button" className="cop-primary" onClick={beginArea}>Dibujar zona</button>
              <button type="button" className="cop-secondary" onClick={() => finishArea({ lng: scenario.incident.center[0], lat: scenario.incident.center[1], radiusM: 3000 })}>Entorno 3 km</button>
            </div>
          </>
        )}
      </section>

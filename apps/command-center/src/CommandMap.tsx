...
export function CommandMap({ token, citizens, fires, zones, selectedId, layers, onSelect, projection, forecast, zoneExposure, horizon, marginM, route, focusTarget, onCenterSelect, showWind, windDirection, windKmh, callArea, areaIds, drawingArea, onAreaChange, onAreaComplete, recommended, units, onUnitSelect, fireCells, centers, incident }: Props) {
  ...
  useEffect(() => {
    ...
    const onLoad = () => {
      ...
      map.addLayer({ id: 'police-car', type: 'symbol', source: 'units', filter: ['==', ['get', 'kind'], 'police'], layout: { 'icon-image': policeCarImage(map.getBearing()), 'icon-size': ['interpolate', ['linear'], ['zoom'], 10, 0.68, 14, 0.9, 17, 1], 'icon-anchor': 'center', 'icon-pitch-alignment': 'viewport', 'icon-rotation-alignment': 'viewport', 'icon-allow-overlap': true, 'icon-ignore-placement': true } })
      map.addLayer({ id: 'ambulance-vehicle', type: 'symbol', source: 'units', filter: ['==', ['get', 'kind'], 'ambulance'], layout: { 'icon-image': 'ambulance-marker', 'icon-size': ['interpolate', ['linear'], ['zoom'], 10, 0.64, 14, 0.82, 17, 0.93], 'icon-rotate': ['get', 'heading'], 'icon-anchor': 'center', 'icon-pitch-alignment': 'map', 'icon-rotation-alignment': 'map', 'icon-allow-overlap': true, 'icon-ignore-placement': true } })

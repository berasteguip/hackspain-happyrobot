...
test('la tarjeta de HappyRobot enseña el despacho del vehículo pulsado: lo real encendido, lo previsto discontinuo', async () => {
  ...
  const base = { id: 'u-1', callSign: 'A-01', kind: 'ambulance', mission: 'dispatch', revision: 2, agent: 'Operador · demo', summary: 'Operador · demo redirige A-01 desde su posición actual hacia Rosa Gil.', origin: 'Hospital del Prado', target: 'Rosa Gil', distanceKm: 6.2, escalated: false }
  ...
  const toolState = (html, id) => html.match(new RegExp(`data-state="([a-z]+)"[^>]*data-demo="hr-unit-node" data-demo-id="${id}"`))?.[1]
  ...
  assert.ok(onWay.includes('hr-run-state active') && onWay.includes('En camino · 7 min'), 'la pill de arriba dice el estado real y la ETA')
  for (const label of ['Petición de medio', 'Medio libre más cercano', 'Ruta hasta el acceso', 'Aprobación del mando', 'Llamada al conductor', 'Vigilar la ETA frente al fuego', 'Parte al colgar']) assert.ok(onWay.includes(label), label)
  for (const word of ['Enlace', 'Censo', 'Resultado', 'Relevo']) assert.ok(onWay.includes(`<strong>${word}</strong>`), word)
  assert.equal(stepState(onWay, 'hook'), 'done')
  assert.equal(stepState(onWay, 'pick'), 'done')
  assert.equal(stepState(onWay, 'route'), 'active')
  assert.ok(onWay.includes('redirige A-01 desde su posición actual') && onWay.includes('Revisión 2'), 'petición y elección cuentan el despacho real')
  assert.ok(onWay.includes('6,2 km · llega en 7 min.') && onWay.includes('<b>7 min</b>'), 'la ruta lleva distancia y ETA')
  for (const id of ['approve', 'driver', 'watch', 'report']) assert.equal(stepState(onWay, id), 'mock', `${id} es previsto`)
  for (const id of ['link', 'census', 'result', 'handoff']) assert.equal(toolState(onWay, id), 'mock', `la tool ${id} es prevista`)
  assert.ok(onWay.includes('data-edge="still"'), 'la arista hacia lo previsto no corre')
  const arrived = render({ ...base, status: 'on_scene' })
  assert.equal(stepState(arrived, 'route'), 'done')
  ...
  assert.equal(stepState(held, 'route'), 'error')
  ...
  for (const id of ['hook', 'pick', 'route']) assert.equal(stepState(patrol, id), 'idle', `${id} en reposo en patrulla`)
  assert.ok(patrol.includes('Patrullando') && patrol.includes('Sin tarea asignada'), 'en patrulla la pill lo dice')
  const escalated = render({ ...base, status: 'en_route', etaMin: 3, agent: 'HappyRobot · Escalada', escalated: true })
  assert.equal(stepState(escalated, 'approve'), 'done', 'la aprobación viene hecha del run de escalada')
  assert.equal(stepState(escalated, 'driver'), 'done', 'y la llamada a organismo también')
  assert.equal(stepState(escalated, 'watch'), 'mock', 'vigilar sigue previsto')
})

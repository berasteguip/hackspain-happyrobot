...
de `backend/api/`. Su razón de existir es el requisito obligatorio del reto **"escenario que se mueve"**:
un caso fijo está descalificado.

## Cómo se arranca

```bash
# la forma que usa `make engine` y la que de verdad funciona desde backend/engine/
...
Las dos funcionan, pero no desde el mismo sitio. `python -m engine.run` con el cwd en `backend/engine/`
no puede resolver (buscaría `backend/engine/engine/`), así que el `make engine` del root —que hace
`cd backend/engine`— cae por su `2>/dev/null ||` a la **forma de script `run.py`**. Esa es la que se usa
en la demo.

El entorno es **Python 3.12** (`uv venv --python 3.12 backend/engine/.venv`,
...
- `to_event_payload` añade `secondary_perimeters` al payload de `/events/fire` cuando hay focos
  por pavesas. **Ese campo no está en el contrato**: `backend/api/` puede ignorarlo sin romper nada, y el
...
```bash
cd backend/engine && .venv/bin/python -m pytest -q     # 39 pasando
```

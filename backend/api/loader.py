"""Carga del escenario desde `backend/data/scenarios/<nombre>.json`.

El fichero lo genera otro componente (`backend/data/generate.py`), así que este módulo es **tolerante**:
...
def load_scenario(state, name: str | None = None) -> dict:
    ...
    if path.is_file():
        ...
        log.warning(
            ...
            "backend/data/generate.py escriba el fichero, `POST /reset` lo carga.",
            path,
        )

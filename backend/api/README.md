# `backend/api/` — Crisis State API

El **cerebro y la única fuente de verdad** del sistema de evacuación guiada. Todo lo demás habla con
ella: el motor de escenario (`backend/engine/`) le manda lo que pasa, HappyRobot le manda lo que la gente
dice por teléfono, el dashboard (`frontend/`) la lee y la corrige, y ella decide **y escribe por qué**.
...
Configuración opcional en `backend/api/.env` (plantilla en [`.env.example`](.env.example)). Sin `.env` arranca
...
### Eventos del mundo (`backend/engine/`, página GPS)
...
en `backend/sim/`, fuera de esta API. Existe para que quien lo llame lea el motivo en vez de un 404.
...
loader.py      carga el escenario JSON de backend/data/scenarios/
...
Reglas duras: el estado vive **en memoria** (reiniciar = escenario limpio), `backend/api/state.jsonl` es

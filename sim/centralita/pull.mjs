...
export async function fetchOutcomeForSession(client, workflowId, sessionId, personId, opts = {}) {
  ...
  while (Date.now() < deadline) {
    try {
      const runsRes = await client.workflows.listRuns(workflowId, { page_size: 20, sort: "desc" });
      const runs = listFrom(runsRes);
      for (const run of runs) {
        const runId = run.id ?? run.run_id;
        if (!runId) continue;
        // Casar el run por el output del trigger (text_session_id o person_id).
        if (matchedRun?.id !== runId) {
          const created = Date.parse(run.created_at ?? run.created ?? 0) || 0;
          ...
          const hit =
            (sessionId && trigData?.text_session_id === sessionId) ||
            (personId && trigData?.person_id === personId) ||
            (sessionId && JSON.stringify(trigData).includes(sessionId));
          if (!hit) continue;
        ...
        const nodes = matchedRun.nodes ?? listFrom(await client.runs.listNodes(runId));

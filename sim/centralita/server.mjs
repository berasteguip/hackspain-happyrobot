...
const ALLOWED_ORIGINS = new Set(["http://127.0.0.1:5173", "http://localhost:5173", "http://127.0.0.1:5174", "http://localhost:5174", ...(env.BRIDGE_ALLOWED_ORIGINS || "").split(",").map(origin => origin.trim()).filter(Boolean)]);
...
function applyOutcome(call, body) {
  ...
  if (body.partial === true) {
    ...
    call.outcome = call.outcome ?? { person_id: body.person_id, partial: true, extracted: {} };
    ...
    call.resultState = typeof body.answered === "boolean" ? "ready" : "failed";
    call.outcomeError = call.resultState === "ready" ? null : "Extracción sin answered válido";
  }
  return call.version;
}
...
function scheduleOutcomePull(call) {
  setTimeout(async () => {
    ...
    try {
      ...
      if (!r.extracted) {
        call.resultState = "failed";
        call.outcomeError = "No se pudo recuperar la extracción en 90 segundos. Requiere revisión.";
      ...
      const payload = {
        ...
        answered: r.answered ?? r.extracted.answered ?? null,
        duration_s: null,
        channel: "chat",
      ...
      if (r.extracted.consent_position === true && !call.consent_position) {
        ...
        call.links.push({ ts: new Date().toISOString(), simulated: true, url: `${PUBLIC_BASE_URL}/track?id=${encodeURIComponent(call.trackingId ?? call.person_id)}` });
      }
      console.log(`[bridge] outcome por pull · ${call.person_id} · run ${r.run_id}`);
    } catch (err) {
      call.resultState = "failed";
      call.outcomeError = "No se pudo recuperar la extracción de HappyRobot.";
...
function pumpQueue() {
  while (running < maxConcurrency && queue.length) {
    ...
    runConversation(client, call.agentData, call.personaData, {
      ...
      onEvent: (kind, detail) => {
        if (kind === "session-created") call[detail.label === "A" ? "agentSessionId" : "personaSessionId"] = detail.session_id;
        console.log(`[${call.person_id}] evento ${kind}`, detail);
      },
    })
...
const server = createServer(async (req, res) => {
  ...
  try {
    if (req.method === "GET" && path === "/health") return send(res, req, 200, { ok: true, channel: "chat", running, queued: queue.length });
    if (req.method === "POST" && path === "/wave/start") {
      if (req.headers.origin && !ALLOWED_ORIGINS.has(req.headers.origin)) return send(res, req, 403, { error: "origin not allowed" });
      const body = await readBody(req);
      const people = Array.isArray(body.people) ? body.people : [];
      if (!people.length || people.length > 4 || people.some(p => typeof p?.agent?.person_id !== "string" || !p.agent.person_id || p.agent.person_id.length > 150 || p.persona?.person_id !== p.agent.person_id)) return send(res, req, 400, { error: "Se requieren entre 1 y 4 fichas válidas" });
      maxConcurrency = Math.min(4, Math.max(1, Math.floor(Number(body.concurrency)) || 4));
      let queued = 0;
      for (const p of people) {
        ...
        calls.set(id, {
          person_id: id,
          trackingId: typeof p.tracking_id === "string" ? p.tracking_id : id,
          state: "queued",
          startedAt: null,
          endedAt: null,
          transcript: [],
    ...
    if (req.method === "POST" && path === "/calls/outcome") {
      ...
      if (body.partial !== true) {
        ...
        if (call.outcome?.extracted?.consent_position === true && !call.links.length) {
          call.consent_position = true;
          call.links.push({ ts: new Date().toISOString(), channel: "sms", simulated: true, url: `${PUBLIC_BASE_URL}/track?id=${encodeURIComponent(call.trackingId ?? body.person_id)}`, source: "extracted" });
...
if (env.BRIDGE_RECOVERY_URL) {
  const source = new URL(env.BRIDGE_RECOVERY_URL);
  if (source.protocol !== "http:" || !["127.0.0.1", "localhost"].includes(source.hostname)) throw new Error("La recuperación solo admite otro puente local");
  const response = await fetch(new URL("/wave/status", source), { signal: AbortSignal.timeout(10000) });
  if (!response.ok) throw new Error(`Recuperación HTTP ${response.status}`);
  const snapshot = await response.json();
  if (!Array.isArray(snapshot.calls) || snapshot.calls.some(call => ["queued", "talking"].includes(call.state))) throw new Error("Espere a que terminen las conversaciones del puente de origen");
  for (const previous of snapshot.calls) {
    const call = { ...previous, outcome: null, resultState: "pending", outcomeError: null, partials: [], agentData: {}, personaData: {} };
    calls.set(call.person_id, call);
    scheduleOutcomePull(call);
  }
  console.log(`[bridge] recuperadas ${calls.size} conversaciones; se vuelve a leer Extract, sin abrir sesiones`);
}
